...
async function diagnosePython() {
    ...
    if (os.platform() === 'win32') {
        console.log('5. Windows注册表Python检查:');
        try {
            const regResult = execSync('reg query "HKEY_LOCAL_MACHINE\\SOFTWARE\\Python" /s', { 
                encoding: 'utf8',
                timeout: 10000
            });
            console.log('   ✅ 注册表中找到Python安装信息');
            
            // 解析注册表输出查找InstallPath
            const lines = regResult.split('\n');
                ...
                if (line.includes('InstallPath') && line.includes('REG_SZ')) {
                    ...
                    if (match) {
                        ...
                        if (fs.existsSync(pythonExe)) {
                            console.log(`   ✅ 注册表Python: ${pythonExe}`);
                        }
                    }
                }
            }
        } catch (regError) {
            console.log(`   ❌ 注册表检查失败: ${regError.message}`);
        }
        console.log('');
    }
    
    // 6. PythonManager测试
    console.log('6. PythonManager测试:');
    try {
