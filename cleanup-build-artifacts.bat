...
REM 移除build目录中的构建产物（保留源文件）
if exist build\ (
    echo 检查 build/ 目录...
    git rm --cached build/*.exe 2>nul
    git rm --cached build/*.msi 2>nul
    git rm --cached build/*.dmg 2>nul
    git rm --cached build/*.deb 2>nul
    git rm --cached build/*.rpm 2>nul
    echo   ℹ build/ 目录已检查
)
